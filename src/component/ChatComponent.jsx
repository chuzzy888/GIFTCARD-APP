...
function ChatComponent() {
  const { sellerContact } = useParams();
  console.log('Seller Contact:', sellerContact);

  const openWhatsAppChat = () => {
    const whatsappLink = `https://wa.me/${sellerContact}`;
    window.open(whatsappLink);
  };

  const openTelegramChat = () => {
    const telegramLink = `https://t.me/${sellerContact}`;
    window.open(telegramLink);
  };

  return (
    <div  className="chat-container p-4 rounded-lg shadow-lg bg-gradient-to-b from-pink-300 to-pink-500 animate__animated animate__fadeInRight">
      <div className="chat-header mb-4">
        <h2 className="text-2xl font-semibold text-white font-serif">Chat with Seller</h2>
      </div>
      <div className="chat-buttons space-y-2">
        <div className='flex'>
          <button
          ...
          <h1 className='text-green-900'><FaSquareWhatsapp /></h1>
        </div>


        <div className='flex'>
        <h1 className='text-blue-900 my-1.5'><BsTelegram/></h1>
        <button
